...
describe('ConsultationRequestDetailPage.vue', () => {
  ...
  it(`displays ${Message.INVALID_CANDIDATE_MESSAGE} if ${Code.INVALID_CANDIDATE} is returned`, async () => {
    const result = createDummyConsultationRequestDetail1(parseInt(routeParam))
    const resp = GetConsultationRequestDetailResp.create(result)
    getConsultationRequestDetailFuncMock.mockResolvedValue(resp)
    const wrapper = mount(ConsultationRequestDetailPage, {
    ...
    const apiErrResp = ApiErrorResp.create(400, ApiError.create(Code.INVALID_CANDIDATE))
    postConsultationRequestAcceptanceFuncMock.mockResolvedValue(apiErrResp)
    const acceptBtn = wrapper.find('[data-test="accept-btn"]')
    await acceptBtn.trigger('click')
    await flushPromises()
    ...
    const errorBelowBtn = wrapper.find('[data-test="error-below-btn"]')
    expect(errorBelowBtn.exists()).toBe(true)
    expect(errorBelowBtn.text()).toContain(Message.INVALID_CANDIDATE_MESSAGE)
    expect(errorBelowBtn.text()).toContain(Code.INVALID_CANDIDATE.toString())
  })

  it(`displays ${Message.USER_DOES_NOT_CHECK_CONFIRMATION_ITEMS_MESSAGE} if ${Code.USER_DOES_NOT_CHECK_CONFIRMATION_ITEMS} is returned`, async () => {
    const result = createDummyConsultationRequestDetail1(parseInt(routeParam))
    const resp = GetConsultationRequestDetailResp.create(result)
    getConsultationRequestDetailFuncMock.mockResolvedValue(resp)
    const wrapper = mount(ConsultationRequestDetailPage, {
    ...
    const apiErrResp = ApiErrorResp.create(400, ApiError.create(Code.USER_DOES_NOT_CHECK_CONFIRMATION_ITEMS))
    postConsultationRequestAcceptanceFuncMock.mockResolvedValue(apiErrResp)
    const acceptBtn = wrapper.find('[data-test="accept-btn"]')
    await acceptBtn.trigger('click')
    await flushPromises()
    ...
    const errorBelowBtn = wrapper.find('[data-test="error-below-btn"]')
    expect(errorBelowBtn.exists()).toBe(true)
    expect(errorBelowBtn.text()).toContain(Message.USER_DOES_NOT_CHECK_CONFIRMATION_ITEMS_MESSAGE)
    expect(errorBelowBtn.text()).toContain(Code.USER_DOES_NOT_CHECK_CONFIRMATION_ITEMS.toString())
  })

  it(`displays ${Message.NON_POSITIVE_CONSULTATION_REQ_ID_MESSAGE} if ${Code.NON_POSITIVE_CONSULTATION_REQ_ID} is returned`, async () => {
    const result = createDummyConsultationRequestDetail1(parseInt(routeParam))
    const resp = GetConsultationRequestDetailResp.create(result)
    getConsultationRequestDetailFuncMock.mockResolvedValue(resp)
    const wrapper = mount(ConsultationRequestDetailPage, {
  ...
  it(`displays ${Message.THE_OTHER_PERSON_ACCOUNT_IS_NOT_AVAILABLE_MESSAGE} if ${Code.THE_OTHER_PERSON_ACCOUNT_IS_NOT_AVAILABLE} is returned`, async () => {
    const result = createDummyConsultationRequestDetail1(parseInt(routeParam))
    const resp = GetConsultationRequestDetailResp.create(result)
    getConsultationRequestDetailFuncMock.mockResolvedValue(resp)
    const wrapper = mount(ConsultationRequestDetailPage, {
    ...
    const apiErrResp = ApiErrorResp.create(400, ApiError.create(Code.THE_OTHER_PERSON_ACCOUNT_IS_NOT_AVAILABLE))
    postConsultationRequestAcceptanceFuncMock.mockResolvedValue(apiErrResp)
    const acceptBtn = wrapper.find('[data-test="accept-btn"]')
    await acceptBtn.trigger('click')
    await flushPromises()
    ...
    const errorBelowBtn = wrapper.find('[data-test="error-below-btn"]')
    expect(errorBelowBtn.exists()).toBe(true)
    expect(errorBelowBtn.text()).toContain(Message.THE_OTHER_PERSON_ACCOUNT_IS_NOT_AVAILABLE_MESSAGE)
    expect(errorBelowBtn.text()).toContain(Code.THE_OTHER_PERSON_ACCOUNT_IS_NOT_AVAILABLE.toString())
  })
