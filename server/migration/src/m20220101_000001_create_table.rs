...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // TODO: これより下はPAY.JPで必要であったもの、またはそれに合わせて設計したもの。収納代行としての実装が進んだ段階で削除する
        let _ = conn
        ...
        let _ = conn
            /* 下記のタイミングで生成される
             *   - コンサルタントが相談申し込みを承認したときに生成される。
             *   - 管理者がユーザーを有効化したとき（相談日時に間に合う場合、決済中止テーブルから情報が移される）
             *   - 管理者が決済再開操作を行ったとき（決済中止テーブルから情報が移される）
             *
             * 下記のタイミングで削除される
        ...
        let _ = conn
             ...
             *   - 管理者がユーザーを有効化したとき（相談日時に間に合う場合、決済テーブルに情報が移される）
             *   - 管理者が決済再開操作を行ったとき（決済中止の後、コンサルタントからの反論で決済を再開すべきと判断した場合、決済再開が実施される）
             */
            // charge_idには、ch_fa990a4c10672a93053a774730b0aのような32文字の文字列が入ることが推定されるが、
            // PAY.JPの実装の変更がある場合に備えてVACHARでなく、TEXTで受ける
            // platform_fee_rate_in_percentageには少数を示す文字列を含む（金額の計算に使うので浮動小数点は使わず、処理に時間をかけないようにnumericも使わない）
