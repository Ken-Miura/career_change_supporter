...
use common::{
    err::Code::InvalidUuidFormat, storage::IDENTITY_IMAGES_BUCKET_NAME,
...
pub(crate) async fn get_identity_images(
    ...
    let op = DownloadIdentityImageOperationImpl { storage_client };
...
impl DownloadIdentityImageOperation for DownloadIdentityImageOperationImpl {
    async fn download_identity_image(&self, key: &str) -> Result<Vec<u8>, ErrResp> {
        let image_binary = self
            .storage_client
            .download_object(IDENTITY_IMAGES_BUCKET_NAME.as_str(), key)
            .await
            .map_err(|e| {
