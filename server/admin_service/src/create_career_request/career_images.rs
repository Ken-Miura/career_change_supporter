...
async fn get_career_images_internal(
    ...
    let image_binary = op.download_career_image(&key).await?;
...
#[async_trait]
trait DownloadCareerImageOperation {
...
mod tests {
    ...
    #[async_trait]
    impl DownloadCareerImageOperation for DownloadCareerImageOperationMock {
        async fn download_career_image(&self, key: &str) -> Result<Vec<u8>, ErrResp> {
    ...
    #[tokio::test]
    async fn get_career_images_internal_success() {
        let user_account_id = String::from("4123");
        let image_name = String::from("bb2c15c13ebc1d7b2b7c8d53cb252b4c");
        let key = format!("{}/{}.png", user_account_id, image_name);
        let image = create_dummy_career_image();
        let op_mock = DownloadCareerImageOperationMock {
            key,
            image: image.clone(),
        };

        let result = get_career_images_internal(user_account_id, image_name, op_mock).await;
    ...
    #[tokio::test]
    async fn get_career_images_internal_fail_invalid_uuid() {
        let user_account_id = String::from("4123");
        // 31桁でinvalid
