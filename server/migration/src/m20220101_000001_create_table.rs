...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let _ = conn
            /* user_account一つに対して、identity_infoは0もしくは1の関係とする。従って、user_account_idを外部キーかつ主キーとする */
        ...
        let _ = conn
            ...
            .execute(sql.stmt(r"GRANT SELECT, DELETE ON ccs_schema.identity_info To user_app;"))
            .await
            .map(|_| ())?;
        let _ = conn
            .execute(
                sql.stmt(r"GRANT SELECT, INSERT, UPDATE ON ccs_schema.identity_info To admin_app;"),
            )
            .await
            .map(|_| ())?;
        let _ = conn
             ...
             */
            .execute(sql.stmt(r"CREATE INDEX identity_info_date_of_birth_idx ON ccs_schema.identity_info (date_of_birth);"))
            .await
            .map(|_| ())?;
        ...
        let _ = conn
            .execute(
                ...
                /* アカウント削除はユーザー自身が行う。そのため削除権限はユーザー (user_app) に付与する */
                sql.stmt(r"GRANT SELECT, DELETE ON ccs_schema.career_info To user_app;"),
            )
            .await
            .map(|_| ())?;
        let _ = conn
            .execute(
                sql.stmt(r"GRANT SELECT, INSERT, UPDATE ON ccs_schema.career_info To admin_app;"),
            )
            .await
            .map(|_| ())?;
        let _ = conn
            .execute(sql.stmt(
                r"GRANT USAGE ON SEQUENCE ccs_schema.career_info_career_info_id_seq TO admin_app;",
            ))
            .await
            .map(|_| ())?;
        ...
        let _ = conn
            .execute(
                sql.stmt(
                    r"GRANT SELECT, INSERT ON ccs_schema.create_identity_info_req To user_app;",
                ),
            )
            .await
            .map(|_| ())?;
        let _ =
            conn.execute(sql.stmt(
                r"GRANT SELECT, DELETE ON ccs_schema.create_identity_info_req To admin_app;",
            ))
            .await
            .map(|_| ())?;
        let _ = conn
