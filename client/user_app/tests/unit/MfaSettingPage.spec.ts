...
describe('MfaSettingPage.vue', () => {
  ...
  it('displays status as 無効 and button as 有効化する', async () => {
    ...
    expect(mfaEnabledLabel.text()).toContain('現在の設定')
  ...
  it(`displays ${Message.UNAUTHORIZED_ON_MFA_SETTING_OPERATION_MESSAGE} if ${Code.UNAUTHORIZED} is returned when 有効化する is clicked`, async () => {
    mfaEnabled = 'false'
    refreshMock.mockResolvedValue(RefreshResp.create())
    const apiErrResp = ApiErrorResp.create(401, ApiError.create(Code.UNAUTHORIZED))
  ...
  it(`displays ${Message.NOT_TERMS_OF_USE_AGREED_YET_ON_MFA_SETTING_OPERATION_MESSAGE} if ${Code.NOT_TERMS_OF_USE_AGREED_YET} is returned when 有効化する is clicked`, async () => {
    mfaEnabled = 'false'
    refreshMock.mockResolvedValue(RefreshResp.create())
    const apiErrResp = ApiErrorResp.create(400, ApiError.create(Code.NOT_TERMS_OF_USE_AGREED_YET))
    postTempMfaSecretFuncMock.mockResolvedValue(apiErrResp)
  ...
  it(`displays ${Message.REACH_TEMP_MFA_SECRET_LIMIT_MESSAGE} if ${Code.REACH_TEMP_MFA_SECRET_LIMIT} is returned when 有効化する is clicked`, async () => {
    mfaEnabled = 'false'
    refreshMock.mockResolvedValue(RefreshResp.create())
    const apiErrResp = ApiErrorResp.create(400, ApiError.create(Code.REACH_TEMP_MFA_SECRET_LIMIT))
    postTempMfaSecretFuncMock.mockResolvedValue(apiErrResp)
    const wrapper = mount(MfaSettingPage, {
      global: {
        stubs: {
    ...
    expect(classes).not.toContain('hidden')
    const resultMessage = alertMessage.text()
    expect(resultMessage).toContain(Message.REACH_TEMP_MFA_SECRET_LIMIT_MESSAGE)
    expect(resultMessage).toContain(Code.REACH_TEMP_MFA_SECRET_LIMIT.toString())
  })
