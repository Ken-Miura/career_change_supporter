            ...
            <li v-for="item in items" v-bind:key="item.user_account_id">
              <div class="mt-4">
                <div class="border border-gray-600 rounded bg-white px-4 py-3 text-black text-xl grid grid-cols-4">
                  ...
                  <button class="col-span-1 bg-gray-600 hover:bg-gray-700 text-white font-bold px-6 py-3 rounded shadow-lg hover:shadow-xl transition duration-200" v-on:click="moveToCreateIdentityRequestDetailPage(item.user_account_id)">詳細を確認する</button>
                </div>
              </div>
            </li>
          </ul>
...
export default defineComponent({
  ...
  setup () {
    ...
    const moveToCreateIdentityRequestDetailPage = async (accountId: number) => {
      await router.push({ name: 'CreateIdentityRequestDetailPage', params: { user_account_id: accountId } })
    }
    const prev = async () => {
