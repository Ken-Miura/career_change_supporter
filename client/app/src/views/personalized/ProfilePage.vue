  ...
  <div class="bg-gradient-to-r from-gray-500 to-gray-900 bo min-h-screen pt-12 md:pt-20 pb-6 px-2 md:px-0" style="font-family:'Lato',sans-serif;">
    <main>
      ...
      <div class="flex flex-col justify-center bg-white max-w-4xl mx-auto p-8 md:p-12 my-10 rounded-lg shadow-2xl">
        ...
        <div v-if="identity !== null" class="m-4 text-2xl grid grid-cols-6">
          <div class="mt-2 justify-self-start col-span-2">名前</div><div class="justify-self-start col-span-4">{{identity.last_name}} {{identity.first_name}}</div>
          <div class="mt-2 justify-self-start col-span-2">フリガナ</div><div class="justify-self-start col-span-4">{{identity.last_name_furigana}} {{identity.first_name_furigana}}</div>
          <div class="mt-2 justify-self-start col-span-2">性別</div><div v-if="identity.sex === 'male'" class="justify-self-start col-span-4">男性</div><div v-else class="justify-self-start col-span-4">女性</div>
          <div class="mt-2 justify-self-start col-span-2">生年月日</div><div class="justify-self-start col-span-4">{{identity.date_of_birth.year}}年{{identity.date_of_birth.month}}月{{identity.date_of_birth.day}}日</div>
          <div class="mt-2 justify-self-start col-span-6">住所</div>
          <div class="mt-2 ml-3 justify-self-start col-span-2">都道府県</div><div class="justify-self-start col-span-4">{{identity.prefecture}}</div>
          <div class="mt-2 ml-3 justify-self-start col-span-2">市区町村</div><div class="justify-self-start col-span-4">{{identity.city}}</div>
          <div class="mt-2 ml-3 justify-self-start col-span-2">番地</div><div class="justify-self-start col-span-4">{{identity.address_line1}}</div>
          <div v-if="identity.address_line2 !== null" class="mt-2 ml-3 justify-self-start col-span-2">建物名・部屋番号</div><div v-if="identity.address_line2 !== null" class="justify-self-start col-span-4">{{identity.address_line2}}</div>
          <div class="mt-2 justify-self-start col-span-2">電話番号</div><div class="justify-self-start col-span-4">{{identity.telephone_number}}</div>
        </div>
