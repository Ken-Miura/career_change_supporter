...
describe('MfaPage.vue', () => {
  ...
  it('moves admin-menu if pass code check is successfull', async () => {
  ...
  it(`displays alert message ${Message.INVALID_PASS_CODE_FORMAT_MESSAGE} if ${Code.INVALID_PASS_CODE_FORMAT} is returned`, async () => {
    const apiErrResp = ApiErrorResp.create(400, ApiError.create(Code.INVALID_PASS_CODE_FORMAT))
    postPassCodeFuncMock.mockResolvedValue(apiErrResp)
    const wrapper = mount(MfaPage, {
      global: {
        stubs: {
    ...
    expect(classes).not.toContain('hidden')
    const resultMessage = alertMessage.text()
    expect(resultMessage).toContain(Message.INVALID_PASS_CODE_FORMAT_MESSAGE)
    expect(resultMessage).toContain(Code.INVALID_PASS_CODE_FORMAT.toString())
  })
