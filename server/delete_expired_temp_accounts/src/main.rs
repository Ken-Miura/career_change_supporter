...
use common::{
    ...
    util::check_env_vars,
    JAPANESE_TIME_ZONE, KEY_TO_USE_ECS_TASK_ROLE, USE_ECS_TASK_ROLE,
    VALID_PERIOD_OF_TEMP_ACCOUNT_IN_HOUR, WEB_SITE_NAME,
};
...
async fn main_internal() {
    let log_conf = format!(
        "delete_expired_temp_accounts={},common={},sea_orm={}",
        LOG_LEVEL.as_str(),
        LOG_LEVEL.as_str(),
        LOG_LEVEL.as_str()
    );
    ...
    let deleted_num = result.unwrap_or_else(|e| {
        error!("failed to delete expired temp accounts: {}", e);
...
async fn delete_expired_temp_accounts(
    ...
    for expired_temp_account in expired_temp_accounts {
        let result = op
            ...
            .await;
        if result.is_err() {
            error!("failed delete_temp_account: {:?}", result);
