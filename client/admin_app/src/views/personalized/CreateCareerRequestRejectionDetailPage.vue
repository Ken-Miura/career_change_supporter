  ...
  <div class="bg-gradient-to-r from-gray-500 to-gray-900 min-h-screen pt-12 md:pt-20 pb-6 px-2 md:px-0" style="font-family:'Lato',sans-serif;">
    ...
    <main v-else>
      <div class="flex flex-col justify-center bg-white max-w-4xl mx-auto p-8 md:p-12 my-10 rounded-lg shadow-2xl">
        <h3 data-test="title" class="font-bold text-2xl">職務経歴確認依頼拒否理由</h3>
...
import { usePostCreateCareerRequestRejection } from '@/util/personalized/create-career-request-rejection-detail/usePostCreateCareerRequestRejection'
...
export default defineComponent({
  ...
  setup () {
    ...
    const submitRejectionReason = async () => {
      try {
        const response = await postCreateCareerRequestRejectionFunc(parseInt(createCareerReqId), rejectionReason.value)
        if (!(response instanceof PostCreateCareerRequestRejectionResp)) {
        ...
        await router.push('/create-career-request-rejection')
      } catch (e) {
        error.exists = true
