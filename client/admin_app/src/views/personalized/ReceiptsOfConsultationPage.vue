<template>
  ...
  <div class="bg-gradient-to-r from-gray-500 to-gray-900 min-h-screen pt-12 lg:pt-20 pb-6 px-2 lg:px-0" style="font-family:'Lato',sans-serif;">
    <div v-if="!getReceiptsOfConsultationDone" class="m-6">
      <WaitingCircle />
    </div>
    <main v-else>
      ...
      <div v-else>
        <div data-test="list" class="flex flex-col justify-center bg-white max-w-4xl mx-auto p-8 lg:p-12 my-10 rounded-lg shadow-2xl">
          <h3 class="font-bold text-xl lg:text-2xl">領収書リスト</h3>
            <ul>
              <li v-for="item in items" v-bind:key="item.consultation_id">
                <div class="mt-6">
                  ...
                  <div class="my-1 lg:my-2 justify-self-start col-span-1">口座番号</div><div class="my-1 lg:my-2 justify-self-start col-span-2">{{ item.account_number }}</div>
                  <div class="my-1 lg:my-2 justify-self-start col-span-1">口座名義人</div><div class="my-1 lg:my-2 justify-self-start col-span-2">{{ item.account_holder_name }}</div>
                  <div class="my-1 lg:my-2 justify-self-start col-span-1">報酬振込者</div><div class="my-1 lg:my-2 justify-self-start col-span-2">{{ item.withdrawal_confirmed_by }}</div>
                  <div class="my-1 lg:my-2 justify-self-start col-span-1">報酬振込日時</div><div class="my-1 lg:my-2 justify-self-start col-span-2">{{ item.created_at }}</div>
                </div>
              </div>
            </li>
          </ul>
...
export default defineComponent({
  ...
  setup () {
    ...
    const items = ref([] as ReceiptOfConsultation[])
    const prevDisabled = computed(() => page.value <= 0)
    const nextDisabled = computed(() => items.value.length < perPage.value)
    const prev = async () => {
      await router.push(`/receipts-of-consultation?page=${(page.value - 1)}&per-page=${perPage.value}`)
    }
    const next = async () => {
      await router.push(`/receipts-of-consultation?page=${(page.value + 1)}&per-page=${perPage.value}`)
    }
    ...
    const getItems = async (page: number, perPage: number) => {
      try {
        const response = await getReceiptsOfConsultationFunc(page, perPage)
        if (!(response instanceof GetReceiptsOfConsultationResp)) {
