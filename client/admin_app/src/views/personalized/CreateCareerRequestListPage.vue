...
import { useGetCreateCareerRequests } from '@/util/personalized/create-career-request-list/useGetCreateCareerRequests'
...
export default defineComponent({
  ...
  setup () {
    const items = ref([] as CreateCareerRequestItem[])
    ...
    const getItems = async (page: number) => {
      try {
        const response = await getCreateCareerRequestsFunc(page, getNumOfItems())
        if (response instanceof GetCreateCareerRequestsResp) {
    ...
    return {
      ...
      waitingRequestDone,
      items,
      moveToCreateCareerRequestDetailPage
    }
  }
})
</script>
