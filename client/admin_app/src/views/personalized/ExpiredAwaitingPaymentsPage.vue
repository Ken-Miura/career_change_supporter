            ...
            <ul>
              <li v-for="item in items" v-bind:key="item.consultation_id">
                <div class="mt-6">
                  ...
                  <div class="my-1 lg:my-2 justify-self-start col-span-1">入金者</div><div v-if="item.sender_name" class="my-1 lg:my-2 justify-self-start col-span-2">{{ item.sender_name }}</div><div v-else class="my-1 lg:my-2 justify-self-start col-span-2">既に身分情報が削除されています</div>
...
import { useGetExpiredAwaitingPayments } from '@/util/personalized/awaiting-payment/useGetExpiredAwaitingPayments'
...
export default defineComponent({
  ...
  setup () {
    ...
    const prev = async () => {
      await router.push(`/expired-awaiting-payments?page=${(page.value - 1)}&per-page=${perPage.value}`)
    }
    const next = async () => {
      await router.push(`/expired-awaiting-payments?page=${(page.value + 1)}&per-page=${perPage.value}`)
    }
    ...
    const getItems = async (page: number, perPage: number) => {
      try {
        const response = await getExpiredAwaitingPaymentsFunc(page, perPage)
        if (!(response instanceof GetExpiredAwaitingPaymentsResp)) {
