...
pub(crate) async fn get_consultations_by_consultant_id(
    ...
    get_consultations_by_consultant_id_internal(query.consultant_id, op).await
}

async fn get_consultations_by_consultant_id_internal(
    consultant_id: i64,
    op: impl ConsultationsOperation,
) -> RespResult<ConsultationsResult> {
    validate_account_id_is_positive(consultant_id)?;
    let consultations = op.get_consultations_by_consultant_id(consultant_id).await?;
    Ok((StatusCode::OK, Json(ConsultationsResult { consultations })))
}

#[async_trait]
trait ConsultationsOperation {
    async fn get_consultations_by_consultant_id(
        &self,
        consultant_id: i64,
...
impl ConsultationsOperation for ConsultationsOperationImpl {
    async fn get_consultations_by_consultant_id(
        ...
        let models = entity::consultation::Entity::find()
            ...
            .map_err(|e| {
                error!(
                    "failed to filter consultation (consultant_id: {}): {}",
                    consultant_id, e
                );
                unexpected_err_resp()
            })?;
...
mod tests {
    ...
    fn create_dummy_consultation1(consultant_id: i64) -> Consultation {
        Consultation {
            consultation_id: 1,
            user_account_id: 510,
            consultant_id,
    ...
    fn create_dummy_consultation2(consultant_id: i64) -> Consultation {
        Consultation {
            consultation_id: 2,
    ...
    async fn get_consultations_by_consultant_id_internal_success_1_result() {
        let consultant_id = 64431;
        let consultation1 = create_dummy_consultation1(consultant_id);
        let op_mock = ConsultationsOperationMock {
            consultant_id,
            consultations: vec![consultation1.clone()],
        };

        let result = get_consultations_by_consultant_id_internal(consultant_id, op_mock).await;
    ...
    async fn get_consultations_by_consultant_id_internal_success_2_results() {
        let consultant_id = 64431;
        let consultation1 = create_dummy_consultation1(consultant_id);
        let consultation2 = create_dummy_consultation2(consultant_id);
        let op_mock = ConsultationsOperationMock {
            consultant_id,
    ...
    async fn get_consultations_by_consultant_id_internal_success_no_result() {
        let consultant_id = 64431;
        let op_mock = ConsultationsOperationMock {
            consultant_id,
            consultations: vec![],
    ...
    #[tokio::test]
    async fn get_consultations_by_consultant_id_internal_fail_user_account_id_is_zero() {
        let consultant_id = 0;
        let op_mock = ConsultationsOperationMock {
            consultant_id,
            consultations: vec![],
    ...
    #[tokio::test]
    async fn get_consultations_by_consultant_id_internal_fail_user_account_id_is_negative() {
        let consultant_id = -1;
        let op_mock = ConsultationsOperationMock {
            consultant_id,
            consultations: vec![],
