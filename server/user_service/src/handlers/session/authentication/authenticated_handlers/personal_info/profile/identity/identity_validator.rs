...
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const LAST_NAME_FURIGANA_MIN_LENGTH: usize = 1;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const LAST_NAME_FURIGANA_MAX_LENGTH: usize = 64;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const FIRST_NAME_FURIGANA_MIN_LENGTH: usize = 1;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const FIRST_NAME_FURIGANA_MAX_LENGTH: usize = 64;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const MIN_AGE_REQUIREMENT: i32 = 18;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const CITY_MIN_LENGTH:
    usize = 1;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const CITY_MAX_LENGTH:
    usize = 32;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const ADDRESS_LINE1_MIN_LENGTH: usize = 1;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const ADDRESS_LINE1_MAX_LENGTH: usize = 128;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const ADDRESS_LINE2_MIN_LENGTH: usize = 1;
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) const ADDRESS_LINE2_MAX_LENGTH: usize = 128;
...
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile) fn validate_identity(
    identity: &Identity,
...
pub(in crate::handlers::session::authentication::authenticated_handlers::personal_info::profile)
...
mod tests {
    ...
    #[test]
    fn validate_identity_returns_err_if_129_chars_address_line1_is_passed() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line1_is_symbol_except_hyphen() {
        let mut identity_list = Vec::with_capacity(SYMBOL_WITH_OUT_HYPHEN_SET.len());
        for s in SYMBOL_WITH_OUT_HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                first_name: "太郎".to_string(),
                last_name_furigana: "ヤマダ".to_string(),
        ...
        for id in identity_list {
            let err = validate_identity(&id, &current_date).expect_err("failed to get Err");
            assert_eq!(
                IdentityValidationError::IllegalCharInAddressLine1(id.address_line1.to_string()),
                err
            );
        }
    }
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line1_includes_symbol_except_hyphen() {
        let mut identity_list = Vec::with_capacity(SYMBOL_WITH_OUT_HYPHEN_SET.len());
        for s in SYMBOL_WITH_OUT_HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                prefecture: "東京都".to_string(),
                city: "町田市".to_string(),
                address_line1: "森野".to_string() + s + "２−２−２２",
                address_line2: Some("サーパスマンション　１０１号室".to_string()),
                telephone_number: "09012345678".to_string(),
            };
            identity_list.push(identity);
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line1_ends_with_symbol_except_hyphen() {
        let mut identity_list = Vec::with_capacity(SYMBOL_WITH_OUT_HYPHEN_SET.len());
        for s in SYMBOL_WITH_OUT_HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                prefecture: "東京都".to_string(),
                city: "町田市".to_string(),
                address_line1: "森野２−２−２２".to_string() + s,
                address_line2: Some("サーパスマンション　１０１号室".to_string()),
                telephone_number: "09012345678".to_string(),
            };
            identity_list.push(identity);
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line1_starts_with_symbol_except_hyphen() {
        let mut identity_list = Vec::with_capacity(SYMBOL_WITH_OUT_HYPHEN_SET.len());
        for s in SYMBOL_WITH_OUT_HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                prefecture: "東京都".to_string(),
                city: "町田市".to_string(),
                address_line1: s.to_string() + "森野２−２−２２",
                address_line2: Some("サーパスマンション　１０１号室".to_string()),
                telephone_number: "09012345678".to_string(),
            };
            identity_list.push(identity);
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line1_is_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line1_includes_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line1_ends_with_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line1_starts_with_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_ok_if_1_char_address_line2_is_passed() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_returns_ok_if_128_char_address_line2_is_passed() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_empty_address_line2_is_passed() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_129_chars_address_line2_is_passed() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_accepts_hypen_on_address_line2() {
        let mut identity_list = Vec::with_capacity(HYPHEN_SET.len());
        for s in HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                city: "町田市".to_string(),
                address_line1: "森野２ー２ー２２".to_string(),
                address_line2: Some(s.to_string()),
                telephone_number: "09012345678".to_string(),
            };
            identity_list.push(identity);
        }
    ...
    #[test]
    fn validate_identity_accepts_space_on_address_line2() {
        let mut identity_list = Vec::with_capacity(SPACE_SET.len());
        for s in SPACE_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_accepts_number_on_address_line2() {
        let mut identity_list = Vec::with_capacity(NUMBER_SET.len());
        for s in NUMBER_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line2_is_symbol_except_hyphen() {
        let mut identity_list = Vec::with_capacity(SYMBOL_WITH_OUT_HYPHEN_SET.len());
        for s in SYMBOL_WITH_OUT_HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
        ...
        for id in identity_list {
            let err = validate_identity(&id, &current_date).expect_err("failed to get Err");
            assert_eq!(
                IdentityValidationError::IllegalCharInAddressLine2(
                    id.address_line2.expect("failed to get value").to_string()
                ),
                err
            );
        }
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line2_includes_symbol_except_hyphen() {
        let mut identity_list = Vec::with_capacity(SYMBOL_WITH_OUT_HYPHEN_SET.len());
        for s in SYMBOL_WITH_OUT_HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                city: "町田市".to_string(),
                address_line1: "森野２−２−２２".to_string(),
                address_line2: Some("サーパス".to_string() + s + "マンション　１０１号室"),
                telephone_number: "09012345678".to_string(),
            };
            identity_list.push(identity);
        }
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line2_ends_with_symbol_except_hyphen() {
        let mut identity_list = Vec::with_capacity(SYMBOL_WITH_OUT_HYPHEN_SET.len());
        for s in SYMBOL_WITH_OUT_HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                city: "町田市".to_string(),
                address_line1: "森野２−２−２２".to_string(),
                address_line2: Some("サーパスマンション　１０１号室".to_string() + s),
                telephone_number: "09012345678".to_string(),
            };
            identity_list.push(identity);
        }
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line2_starts_with_symbol_except_hyphen() {
        let mut identity_list = Vec::with_capacity(SYMBOL_WITH_OUT_HYPHEN_SET.len());
        for s in SYMBOL_WITH_OUT_HYPHEN_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                city: "町田市".to_string(),
                address_line1: "森野２−２−２２".to_string(),
                address_line2: Some(s.to_string() + "サーパスマンション　１０１号室"),
                telephone_number: "09012345678".to_string(),
            };
            identity_list.push(identity);
        }
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line2_is_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line2_includes_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line2_ends_with_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_address_line2_starts_with_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_accepts_10_digits_telephone_number() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_accepts_13_digits_telephone_number() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_rejects_9_digits_telephone_number() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_rejects_14_digits_telephone_number() {
        let identity = Identity {
            last_name: "山田".to_string(),
            first_name: "太郎".to_string(),
            last_name_furigana: "ヤマダ".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_is_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
        ...
        for id in identity_list {
            let err = validate_identity(&id, &current_date).expect_err("failed to get Err");
            assert_eq!(
                IdentityValidationError::InvalidTelNumFormat(id.telephone_number.to_string()),
                err
            );
        }
    }
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_includes_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                address_line1: "森野２−２−２２".to_string(),
                address_line2: Some("サーパスマンション　１０１号室".to_string()),
                telephone_number: "090".to_string() + s + "12345678",
            };
            identity_list.push(identity);
        }
        let current_date = NaiveDate::from_ymd_opt(2022, 1, 30).expect("failed to get NaiveDate");
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_ends_with_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                address_line1: "森野２−２−２２".to_string(),
                address_line2: Some("サーパスマンション　１０１号室".to_string()),
                telephone_number: "09012345678".to_string() + s,
            };
            identity_list.push(identity);
        }
        let current_date = NaiveDate::from_ymd_opt(2022, 1, 30).expect("failed to get NaiveDate");
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_starts_with_control_char() {
        let mut identity_list = Vec::with_capacity(CONTROL_CHAR_SET.len());
        for s in CONTROL_CHAR_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
                ...
                address_line1: "森野２−２−２２".to_string(),
                address_line2: Some("サーパスマンション　１０１号室".to_string()),
                telephone_number: s.to_string() + "09012345678",
            };
            identity_list.push(identity);
        }
        let current_date = NaiveDate::from_ymd_opt(2022, 1, 30).expect("failed to get NaiveDate");
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_is_symbol() {
        let mut identity_list = Vec::with_capacity(SYMBOL_SET.len());
        for s in SYMBOL_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_includes_symbol() {
        let mut identity_list = Vec::with_capacity(SYMBOL_SET.len());
        for s in SYMBOL_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_ends_with_symbol() {
        let mut identity_list = Vec::with_capacity(SYMBOL_SET.len());
        for s in SYMBOL_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_starts_with_symbol() {
        let mut identity_list = Vec::with_capacity(SYMBOL_SET.len());
        for s in SYMBOL_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_is_space() {
        let mut identity_list = Vec::with_capacity(SPACE_SET.len());
        for s in SPACE_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_includes_space() {
        let mut identity_list = Vec::with_capacity(SPACE_SET.len());
        for s in SPACE_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_starts_with_space() {
        let mut identity_list = Vec::with_capacity(SPACE_SET.len());
        for s in SPACE_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
    ...
    #[test]
    fn validate_identity_returns_err_if_telephone_number_ends_with_space() {
        let mut identity_list = Vec::with_capacity(SPACE_SET.len());
        for s in SPACE_SET.iter() {
            let identity = Identity {
                last_name: "山田".to_string(),
